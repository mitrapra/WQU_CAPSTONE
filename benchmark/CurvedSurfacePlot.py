...
class CurvedSurfacePlot:
    def __init__(self, CAGR_values, ALPHA_values, STABILITY_values):
        ...
        """
        # Add a small amount of noise to the input data to avoid co-planar points
        self.noise_level 	  = 1e-10
        self.CAGR_values      = CAGR_values  # Store CAGR values
        self.ALPHA_values     = ALPHA_values + self.noise_level * np.random.randn(*ALPHA_values.shape) # Store ALPHA values
        self.STABILITY_values = STABILITY_values + self.noise_level * np.random.randn(*STABILITY_values.shape) # Store STABILITY values 
